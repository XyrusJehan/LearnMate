    ...
    if (empty($groupName)) {
        ...
        if (!isset($error)) {
            ...
            try {
                ...
                $pdo->commit();
                header('Location: student_group.php?created=' . $groupId);
                exit();
...
// Fetch groups the student is in (with admin status)
    $stmt = $pdo->prepare("
        SELECT g.*, 
...
// Fetch public groups (excluding ones student is already in)
    $stmt = $pdo->prepare("
        SELECT g.*, 
              (SELECT COUNT(*) FROM group_members WHERE group_id = g.id) as member_count,
              MAX(CASE WHEN gm.user_id = ? AND gm.is_admin = 1 THEN 1 ELSE 0 END) as is_admin
    ...
    <main class="main-content">
      ...
      <div class="modal" id="createGroupModal">
        <div class="modal-content">
          ...
          <form action="student_group.php" method="POST" enctype="multipart/form-data" id="groupForm">
        ...
        <div class="groups-grid">
          <?php foreach ($userGroups as $group): ?>
            <div class="group-card">
              <a href="student_group_details.php?id=<?php echo $group['id']; ?>" class="group-card-link">
