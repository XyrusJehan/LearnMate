...
// Get all PDFs for the modal (only user's own PDFs)
...
// Get student's classes for the sidebar dropdown
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['delete_flashcard'])) {
        ...
        header("Location: flashcard.php" . ($activeFolderId ? "?folder_id=$activeFolderId" : ""));
        exit();
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['move_flashcards'])) {
        ...
        try {
            ...
            header("Location: flashcard.php" . ($targetFolderId ? "?folder_id=$targetFolderId" : ""));
            exit();
        } catch (Exception $e) {
            $conn->rollback();
  ...
  <div class="app-container">
    ...
    <aside class="sidebars">
      ...
      <div class="nav-section">
        ...
        <a href="flashcard.php" class="nav-item active">
    ...
    <main class="main-content">
      ...
      <div class="manual-flashcard-container">
        ...
        <div class="manual-flashcard-wrapper" style="display: flex; gap: 20px;">
          <!-- Flashcard Folders Sidebar (Left) -->
          <div class="flashcard-sidebar">
            <div class="sidebar-header">
              <h3 style="display: flex; align-items: center; gap: 8px; font-size: 1rem; margin: 0; font-weight: 600; color: var(--text-dark);"><i class="fas fa-folder"></i> My Folders</h3>
            ...
            <div class="folder-list-scroll">
              <ul class="folder-list">
                <li class="folder-item">
                  <a href="flashcard.php" class="folder-link<?php echo !$activeFolderId ? ' active' : '' ?>">
                  </a>
                </li>
                <?php foreach ($allFolders as $folder): ?>
                  <li class="folder-item" style="position: relative; display: flex; align-items: center; justify-content: space-between;">
                    <a href="flashcard.php?folder_id=<?php echo $folder['id']; ?>" 
                       class="folder-link<?php echo $activeFolderId == $folder['id'] ? ' active' : '' ?>">
                      <i class="fas fa-folder"></i> <?php echo htmlspecialchars($folder['name']); ?>
                    </a>
                    <div class="dropdown folder-dropdown" style="margin-left: 4px;">
                      <button class="dropdown-btn folder-dropdown-btn" style="background: none; border: none; color: var(--text-light); cursor: pointer; padding: 5px; font-size: 1.2rem;" title="More actions" tabindex="0"><i class="fas fa-ellipsis-v"></i></button>
                      <div class="dropdown-content folder-dropdown-content">
                        <a href="#" class="delete-folder-btn" data-id="<?php echo $folder['id']; ?>"><i class="fas fa-trash"></i> Delete</a>
                        <a href="#" class="archive-folder-btn" data-id="<?php echo $folder['id']; ?>"><i class="fas fa-archive"></i> Archive</a>
                      </div>
                    </div>
                  </li>
                <?php endforeach; ?>
          ...
          <div class="manual-flashcard-content" style="flex: 1; min-width: 0;">
            ...
            <form class="manual-flashcard-form bordered-form" method="POST" action="flashcard.php">
              ...
              <div class="form-actions-row" style="margin-top: 16px; display: flex; align-items: center; gap: 12px;">
                ...
                <span style="margin-left: 8px; color: var(--text-light); font-size: 0.95em; white-space: nowrap;"></span>
              </div>
            </form>
            <?php if ($activeFolderId && !empty($flashcards)): ?>
              <form method="POST" action="export_pdf.php" target="_blank" style="margin-top: 16px;">
          ...
          <div class="flashcard-list-sidebar" style="min-width: 320px; max-width: 400px; width: 100%; display: flex; flex-direction: column; height: 80vh;">
            <div class="flashcards-scroll-area" style="flex: 1 1 auto; overflow-y: auto; max-height: 100%; min-height: 0; display: flex; flex-direction: column;">
              <div class="flashcards-sticky-header" style="position: sticky; top: 0; z-index: 3; background: var(--bg-white); padding-bottom: 10px; margin-bottom: 10px; border-bottom: 1px solid var(--border-light); display: flex; align-items: center; gap: 10px;">
                <h2 style="font-size: 1.1rem; margin: 0;"><i class="fas fa-layer-group"></i> Flashcards in this folder</h2>
              </div>
              <div class="flashcards-container" style="margin-top: 0; background: var(--bg-white); padding: 0; border-radius: 10px; box-shadow: none; width: 100%; transition: background-color var(--transition), box-shadow var(--transition); flex: 1 1 auto;">
          ...
          <?php endif; ?>
          <!-- Move Actions Bar (hidden by default, shown in move mode) -->
          <div class="move-actions-bar" style="display: none; position: fixed; bottom: 0; left: 250px; right: 0; background: #fff; padding: 15px 20px; box-shadow: 0 -2px 8px rgba(16,24,40,0.08); z-index: 1000; align-items: center; justify-content: space-between;">
            ...
            <div class="move-actions-center">
              <button class="move-btn move-submit" id="move-selected-btn" style="background: var(--primary); color: #fff; padding: 8px 20px; border-radius: 6px; font-weight: 600; border: none;">
                <i class="fas fa-arrow-right"></i> Move to
              </button>
            </div>
            <div class="move-actions-right">
              <button class="move-btn move-cancel" id="cancel-move-btn" style="background: none; color: var(--danger); border: 1px solid var(--danger); padding: 8px 20px; border-radius: 6px; font-weight: 600;">
  ...
  <div class="bottom-nav-container">
    <nav class="bottom-nav">
      ...
      <div class="fab-container">
        <a href="flashcard.php" class="fab">
          <i class="fas fa-plus"></i>
        </a>
      </div>
      ...
      <a href="review.php" class="nav-item-mobile active">
        <i class="fas fa-book-open"></i>
        <span>Review</span>
      </a>
